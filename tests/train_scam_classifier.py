"""
Train a machine learning classifier for scam detection.
Uses TF-IDF + Logistic Regression on labeled data.
"""
...
# =====================================================================
# LABELED TRAINING DATA (Extracted from test templates)
# =====================================================================

# 90 SCAM EXAMPLES (labeled as 1)
SCAM_TEXTS = [
    # KYC Scams (10)
...
# 60 LEGITIMATE EXAMPLES (labeled as 0)
LEGIT_TEXTS = [
...
def train_classifier():
    """Train and save the scam classifier."""
    print("=" * 70)
    ...
    labels = [1] * len(SCAM_TEXTS) + [0] * len(LEGIT_TEXTS)  # 1 = scam, 0 = legit
    ...
    print("Top 20 features the model uses for scam detection:")
    tfidf = pipeline.named_steps['tfidf']
    clf = pipeline.named_steps['clf']
    feature_names = tfidf.get_feature_names_out()
    coefficients = clf.coef_[0]
    
    # Get top positive coefficients (most indicative of scams)
    top_indices = coefficients.argsort()[-20:][::-1]
    for i, idx in enumerate(top_indices, 1):
        print(f"  {i:2d}. {feature_names[idx]:20s} (weight: {coefficients[idx]:.3f})")
    ...
    print("No hardcoded keywords - it discovers patterns automatically!")
    print()
    print("Next: Update backend/main.py to use this model")
    print("=" * 70)
